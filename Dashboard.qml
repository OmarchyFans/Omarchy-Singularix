...
Item {
  ...
  readonly property int planApprovalCount: status && status.harness && status.harness.pending_approvals ? status.harness.pending_approvals.length : 0
  ...
  Process {
    ...
    onExited: function(code) {
      ...
      // A helper older than this window does not know update-check and says so.
      ...
      if (code !== 0 && String(updateErr.text || "").indexOf("unknown command") >= 0)
        dash.updateInfo = { mismatch: true, update_available: false, latest: null, notes: [], dismissed: "", cli: "older" }
    }
  }
  function runUpdate() {
    ...
    FocusScope {
      ...
      PanelKeyCatcher {
        ...
        onTextKey: function(t) {
          ...
          else if (t === "2") dash.selectTab("plan")
          else if (t === "3") dash.selectTab("agents")
