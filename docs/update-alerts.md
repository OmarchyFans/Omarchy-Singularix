...
- Kind: keepLoaded panel (`Dashboard.qml`) plus a bar widget (`BarWidget.qml`); helper `bin/omarchy-agent-launcher` (bash).
- Helper commands: `omarchy-agent-launcher update-check [VERSION] [--force]`, `update-dismiss VERSION`, `update-run [all|install]`, all delegating to `lib/update.sh`.
- Version places: `manifest.json` (the helper reads it at run time as `OAL_VERSION`), `CHANGELOG.md`.
- Cache: `~/.cache/omarchy-agent-launcher/update-check.json`. Opt-out: `omarchy-agent-launcher settings set update_check false` (`~/.config/omarchy-agent-launcher/settings.json`).
- The dashboard checks when it opens and shows the banner above the pages; the bar button checks on load and every six hours and shows a dot.
- keepLoaded: the update terminal offers `omarchy restart shell` at the end, because a keepLoaded panel does not reload cleanly on a file change.
- No mismatch case: `install.sh` symlinks the helper, so the plugin folder is the only copy.
- Tests: the `update` section of `tests/run.sh` runs the check against `file://` fixtures.
