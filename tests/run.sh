...
# Desktop toasts are OFF unless the user turns them on: the bar widget's badge is the default signal.
grep -q "omarchy-notification-send" <<<"$out" && tfail "a blocker must not toast by default"
...
"$L" settings set notify_blockers true >/dev/null; [[ $("$L" settings get notify_blockers) == true ]] || tfail "settings true"
out=$("$L" --dry-run event issue-triage blocker "loud" --level blocker 2>&1) || tfail "event blocker with toasts on"
grep -q "omarchy-notification-send" <<<"$out" || tfail "toast missing once notify_blockers=true"
# ...and when it does toast it must be a normal, self-expiring notification, never a sticky critical one.
grep -q -- "-u normal" <<<"$out" || tfail "an enabled toast must be urgency normal, not critical"
grep -q -- "-u critical" <<<"$out" && tfail "a blocker toast must never be critical (those never expire)"
grep -qE -- "-t [0-9]+" <<<"$out" || tfail "an enabled toast must carry an expire timeout"
"$L" event issue-triage blocker_cleared "" --key loud >/dev/null
